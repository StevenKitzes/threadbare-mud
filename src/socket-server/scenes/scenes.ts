...
import { appendAlsoHereString as appendAlsoHereString } from '../../utils/appendAlsoHereString';
  ...
  handleSceneCommand: (handlerOptions: HandlerOptions): boolean => {
    ...
    if (command === 'look') {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} looks around the room.`,
        options: { other: true }
      });
    ...
    if (command.includes('look drawers')) {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} investigates a chest of drawers.`,
        options: { other: true }
      });

      socket.emit('game-text', {
        gameText: 'You see a sturdy, but otherwise ordinary, chest of drawers.'
      });
    ...
    if (command.includes('peek drawers')) {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} peeks inside a chest of drawers.`,
        options: { other: true }
      });

      socket.emit('game-text', {
        gameText: 'There is nothing inside.'
      });
    ...
    if (command.includes('look window')) {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} peeks out the window.`,
        options: { other: true }
      });

      socket.emit('game-text', {
        gameText: "The view out the window is dizzying.  A bustling town lies below, far below . . . so far below that it is difficult to make out individual people through the clouds.  Rooftops sprawl away in all directions.  An expanse of farmlands lays beyond, and in the farther distance, mountains and an ocean are visible."
      });
    ...
    if (command.includes('look door')) {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} inspects a heavy wooden door.`,
        options: { other: true }
      });

      socket.emit('game-text', {
        gameText: "The door is near twice as tall as it needs to be for someone to pass comfortably through, and wider than necessary, as well.  A mark of luxury?  You wouldn't think so, considering the crude iron binding that holds it together, or the lack of any ornamentation."
      });
    ...
    if (command.includes('peek door')) {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} sneaks a peek through a heavy wooden door.`,
        options: { other: true }
      });

      socket.emit('game-text', {
        gameText: 'Beyond the door, you can make out what looks to be a fabulous library.  You can see hints of bookshelves and dusty piles of old tomes and stacked scrolls.  The furniture is diverse and elaborate and the room is lit by a sparkling, multi-colored light filtering in through stained glass windows.'
      });
...
scenes.set(SceneIds.A_MARVELOUS_LIBRARY, {
  ...
  handleSceneCommand: (handlerOptions: HandlerOptions): boolean => {
    ...
    if (command === 'look') {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} looks around the library.`,
        options: { other: true }
      });
    ...
    if (command.includes('look books')) {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} peruses the stacks of discarded books.`,
        options: { other: true }
      });

      socket.emit('game-text', {
        gameText: "It is amazing how many books are piled on the tables and desks.  Some have clearly been used more recently than others, as told by the varying thickness of the dust on their covers.  Some lie open, while others have bookmarks tucked between their pages.  The library seems active, if only just."
      });
    ...
    if (command.includes('look scrolls')) {
      socket.to(character.scene_id).emit('game-text', {
        gameText: `${character.name} casts an eye to the scrolls littering the library.`,
        options: { other: true }
      });

      socket.emit('game-text', {
        gameText: "Scrolls lie amid the books.  Some are tied with ribbon, others with twine, others curled shut only because that is how they look to have spent hundreds of their long years.  The runes scrawled over their surfaces provide no hint to the casual observer as to their meanings."
      });
    ...
    if (command.includes('peek heavy door')) {
      ...
      socket.emit('game-text', {
        gameText: "You see the bedroom with satin sheets in which you once awakened with no memory . . ."
      });
