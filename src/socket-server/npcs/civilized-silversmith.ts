...
export function make(): NPC {
  const npc: NPC = {
    ...
    saleItems: [
      items.get(ItemIds.SILVER_CROWN),
      items.get(ItemIds.SILVER_RINGS),
      items.get(ItemIds.INSPIRING_SILVER_SCEPTER),
      items.get(ItemIds.SILVER_DAGGER),
    ],
  ...
  npc.handleNpcCommand = (handlerOptions: HandlerOptions): boolean => {
    ...
    if (command.match(makeMatcher(REGEX_TALK_ALIASES, npc.regexAliases))) {
      ...
      emitSelf([
        `This [civilized silversmith] is one of the politest people you can imagine meeting.  She carries herself with gracious aplomb, and takes pride in her craft without arrogance.  "Welcome, can I interest you in an example of my craft?"`,
