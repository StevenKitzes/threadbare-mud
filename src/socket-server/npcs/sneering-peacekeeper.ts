...
export function make(): NPC {
  const npc: NPC = {
    id: NpcIds.SNEERING_PEACEKEEPER,
    name: "a sneering peacekeeper",
    description: "A [sneering peacekeeper], city guard of Parliament, stalks about on his rounds.  He sneers at anyone who crosses his path.",
    keywords: ['sneering peacekeeper', 'peacekeeper', 'guard', 'city guard'],
    regexAliases: 'peacekeeper|sneering peacekeeper|guard|city guard',
  ...
  npc.getDescription = function (): string {
    if (npc.health < 1) {
      return `The body of ${npc.name} lies lifeless on the ground, no longer sneering.`;
    } else {
    ...
    if (command.match(makeMatcher(REGEX_TALK_ALIASES, npc.regexAliases))) {
      ...
      switch (character.job) {
        ...
        case ClassTypes.skyguard:
          emitSelf(`"No loitering, skin-job!"  ${firstCharToUpper(npc.name)} sneers at your bristling furry patches.`);
          break;
