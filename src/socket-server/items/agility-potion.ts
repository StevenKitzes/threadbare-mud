...
const id: ItemIds = ItemIds.AGILITY_POTION;
const type: ItemTypes = ItemTypes.consumable;
const title: string = "a mysterious, light blue potion";
const description: string = "A [light blue potion] in a fist-sized vial, fizzing madly beneath its cork.";
const keywords: string[] = ['light blue potion'];
...
const handleItemCommand = (handlerOptions: HandlerOptions): boolean => {
  ...
  if (
    ...
    characterUpdate.agility = character.agility + 1;
    ...
    if (writeCharacterData(character.id, characterUpdate)) {
      ...
      emitSelf(`You gulp down ${title} and feel the Lifelight burning you up from the inside out.  When the feeling subsides, you feel your reflexes are sharper than before.`);
      return true;
    }
  }
